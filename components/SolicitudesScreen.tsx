...
export default function SolicitudesScreen() {
  ...
  const filtered = filter === 'enjoyed'
    ? solicitudes.filter(s => s.status === 'approved' && (s.end_date || s.start_date) < todayISO)
    : filter === 'approved'
      ? solicitudes.filter(s => s.status === 'approved' && (s.end_date || s.start_date) >= todayISO)
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0f1520] flex flex-col">
      ...
      {/* Header */}
      <header className="sticky top-0 z-10 bg-white dark:bg-[#151b26] border-b border-slate-200 dark:border-slate-800">
        <div className="max-w-md mx-auto flex items-center gap-3 px-4 h-14">
          ...
          <h1 className="text-base font-bold text-slate-900 dark:text-white flex-1">Mis Solicitudes</h1>
            ...
            {FILTERS.map(f => (
              ...
              >
                ...
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                  filter === f.key ? 'bg-primary/10 text-primary' : 'bg-slate-200 dark:bg-white/10 text-slate-400'
                }`}>
